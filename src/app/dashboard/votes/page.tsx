    ...
    <main className="p-8">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {projectsData.data.map((project) => (
          <div key={project.id} className="flex items-center gap-4 p-4 border rounded-lg shadow-md">
            ...
            <div>
              ...
              <ul className="mt-2">
                {project.attributes.studentList.map((student) => (
                  <li className="text-xs bg-foreground/10 text-foreground inline-block px-2 py-0.5 rounded-full" key={student.id}>{student.name}</li>
                ))}
              </ul>
                <p>Number of votes : {
                  //Find the number of votes for the current project
                  votes.find((vote) => vote.projectId === project.id)?.votes || 0
                  }</p>
                  <Button variant="default" className="mt-4 gap-2" asChild>
                    <Link href={`${process.env.NEXT_PUBLIC_WEBSITE_FRONT_URL}/fr/${project.id}`} target="_blank" rel="noopener noreferrer">
